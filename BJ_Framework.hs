...
-- We need to allow different players with different strategies:
-- 1. Naive Player in the original file
-- 2. Players with Counting card strategies associated with different parameters
...
1. A state monad to keep track of the count
2. How to put in various bets according to the current count
3. How to incorporate card counting with the dealCard function
4. How to change the reshuffle such that the cards are not shuffled for every round?
5. Implement different players according to different strategies
6. Refactor the code so it works for different number of decks of cards
